...
#>
    ...
    $shimPath = Join-Path $env:USERPROFILE '.local\bin\rust-analyzer.cmd'
    $lockFilePath = Join-Path $cacheRoot 'rust-analyzer\ra.lock'
    $transport = if (Get-Command Get-RustAnalyzerTransportStatus -ErrorAction SilentlyContinue) {
    ...
    if ($result.ShimExists) {
        $resolvedPath = Get-Command 'rust-analyzer' -CommandType Application -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty Source
        if ($resolvedPath -and $resolvedPath -eq $result.ShimPath) {
            $result.ShimPriority = $true
        } elseif ($resolvedPath -and $resolvedPath -like '*.cmd') {
            $result.ShimPriority = $true
        }
    }
    ...
    if ($result.Transport -and $result.Transport.LspmuxAvailable -and $result.Transport.Effective -ne 'lspmux') {
        $result.Recommendations += 'lspmux is available but not active for default LSP startup; check CARGOTOOLS_RA_TRANSPORT / RUST_ANALYZER_TRANSPORT.'
    }
