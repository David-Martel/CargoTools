...
#>
    ...
    $Sccache = $false
    $NoSccache = $false
    $sharedCacheRoot = Resolve-WslSharedCacheRoot
    ...
    function Show-Help {
        ...
        Write-Host "  - Use --shared to reuse Windows cache at $sharedCacheRoot" -ForegroundColor Gray
    ...
    $wslCmd = Find-CargoCommandPath -Name 'wsl.exe'
    if (-not $wslCmd) { Write-Error 'wsl.exe not found. Install WSL or add it to PATH.'; return 1 }

    $cwd = (Get-Location).Path
    $wslArgs = @()
    if (-not [string]::IsNullOrWhiteSpace($selectedDistro)) { $wslArgs += @('-d', $selectedDistro) }
    $wslCwd = & $wslCmd @wslArgs wslpath -a $cwd 2>$null
    if (-not $wslCwd) {
        $wslCwd = & $wslCmd @wslArgs wslpath -u $cwd 2>$null
    }
    if (-not $wslCwd) {
    ...
    if ($cacheMode -eq 'native') {
        ...
        if (-not $sharedCacheRoot) {
            Write-Error 'Unable to resolve shared CargoTools cache root for WSL.'
            return 1
        }

        $envBlock.Add("CARGO_HOME=$sharedCacheRoot/wsl-cargo-home")
        $envBlock.Add("RUSTUP_HOME=$sharedCacheRoot/wsl-rustup")
        $envBlock.Add("CARGO_TARGET_DIR=$sharedCacheRoot/cargo-target")
    }
    ...
    if ($useSccache) {
        ...
        if ($cacheMode -eq 'native') {
            ...
            $envBlock.Add("SCCACHE_DIR=$sharedCacheRoot/sccache")
        }
    ...
    $joinedArgs = Convert-ArgsToShell $remaining.ToArray()
    ...
    try {
        & $wslCmd @wslArgs -e bash -lc $command
        return $LASTEXITCODE
    } catch {
