...
#>
    ...
    function Show-Help {
        ...
        Write-Host "  - Uses $sharedCacheRoot\cargo-home as CARGO_HOME" -ForegroundColor Gray
        Write-Host "  - Uses $sharedCacheRoot\cargo-target as CARGO_TARGET_DIR" -ForegroundColor Gray
        Write-Host '  - Picks an existing rust:* image if available, otherwise rust:latest' -ForegroundColor Gray
        Write-Host "  - Optional: --sccache mounts $sharedCacheRoot\sccache to /sccache" -ForegroundColor Gray
    ...
    $dockerExe = Find-CargoCommandPath -Name 'docker'
    if (-not $dockerExe) { Write-Error 'docker not found in PATH. Install Docker Desktop or add docker.exe to PATH.'; return 1 }
    ...
    if ([string]::IsNullOrWhiteSpace($selectedImage)) {
        if ($Zigbuild) {
            ...
            $images = & $dockerExe images --format "{{.Repository}}:{{.Tag}}" 2>$null | Where-Object { $_ -like 'rust:*' }
