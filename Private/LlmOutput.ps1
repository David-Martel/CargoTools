...
function Get-MessageFormatArgs {
    <#
    .SYNOPSIS
    Injects --message-format=json into cargo args when LLM mode is active.
    .PARAMETER PrimaryCommand
    The primary cargo command (build, check, clippy, test, bench, etc.)
    .PARAMETER ArgsList
    ...
    # Don't inject if already specified
...
function ConvertFrom-CargoJson {
    <#
    .SYNOPSIS
    Parses cargo JSON diagnostic lines into structured output.
    .PARAMETER JsonLines
    Raw JSON lines from cargo --message-format=json output.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    foreach ($line in $JsonLines) {
        ...
        if ($obj.reason -eq 'compiler-message') {
            $msg = $obj.message
            if ($msg.level -eq 'error') { $errors++ }
            elseif ($msg.level -eq 'warning') { $warnings++ }
            $diagnostics += [PSCustomObject]@{
                level   = $msg.level
                message = $msg.message
                code    = if ($msg.code) { $msg.code.code } else { $null }
            }
        }
        elseif ($obj.reason -eq 'compiler-artifact') {
            if ($obj.filenames) {
                $artifacts += [PSCustomObject]@{
                    package  = if ($obj.package_id) { ($obj.package_id -split ' ')[0] } else { $null }
...
function Format-LlmBuildSummary {
    <#
    .SYNOPSIS
    Emits a single-line JSON build summary for LLM agents.
    .PARAMETER ExitCode
    Cargo exit code.
    .PARAMETER Command
    Primary cargo command that was run.
    .PARAMETER Duration
    Build duration as TimeSpan.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    $durationSec = if ($Duration) { [Math]::Round($Duration.TotalSeconds, 2) } else { 0 }
    ...
    Write-Host "CARGO_LLM_STATUS: $json"
}
