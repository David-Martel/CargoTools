...
function Initialize-CargoLlmOutput {
    $script:LlmOutputMode = $true
    $script:VerbosityLevel = 1
}

function Write-LlmJson {
    param([hashtable]$Data)
    $Data['ts'] = (Get-Date -Format 'HH:mm:ss')
    # Manual JSON construction to avoid ConvertTo-Json overhead and PS 5.1 compat issues
    $pairs = foreach ($key in $Data.Keys) {
        $val = $Data[$key]
        if ($null -eq $val) { "`"$key`":null" }
        else { "`"$key`":`"$($val -replace '\\','\\' -replace '"','\"')`"" }
    }
...
function Write-CargoStatus {
    ...
    if ($script:LlmOutputMode) {
        Write-LlmJson @{ phase = $Phase; type = $Type; message = $Message }
        return
    }
...
function Write-CargoBuildPhase {
    ...
    param(
        [Parameter(Mandatory)]
        [ValidateSet('Queue', 'Preflight', 'Environment', 'Build', 'PostBuild', 'AutoCopy')]
        [string]$Phase,
    ...
    if ($script:LlmOutputMode) {
        $state = if ($Starting) { 'starting' } elseif ($Complete) { 'complete' } elseif ($Failed) { 'failed' } else { 'unknown' }
        Write-LlmJson @{ phase = $Phase; state = $state }
        return
    }
...
function Format-CargoDiagnostics {
    ...
    $sccacheErrorLog = if ($env:SCCACHE_ERROR_LOG) { $env:SCCACHE_ERROR_LOG } else { Join-Path (Resolve-CacheRoot) 'sccache\error.log' }
  ...
  Troubleshooting:
    ...
    5. Check $sccacheErrorLog for sccache errors
  ============================================
"@
...
function Initialize-CargoVerbosity {
    ...
    foreach ($arg in $Arguments) {
        switch ($arg) {
            ...
            '--llm-output' { Initialize-CargoLlmOutput }
        }
    }
...
function Get-VerbosityArgs {
    ...
    $verbosityArgs = @('-q', '--quiet', '-v', '--verbose', '-vv', '--debug', '--llm-output')
